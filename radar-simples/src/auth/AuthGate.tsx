...
import { hasVerifiedAccess, isInitialAdmin, normalizeEmail, type AccessRole, type AuthorizedUser } from './access';
...
export default function AuthGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => onAuthStateChanged(auth, async (current) => {
    ...
    } catch (error) { console.error('Falha ao verificar autorização', error); setMessage('Não foi possível confirmar sua autorização. Tente novamente.'); setState('error'); }
  }), []);
...
function AccessScreen({ state, message, email, onLogin, onLogout }: { state: GateState; message: string; email?: string | null; onLogin: () => void; onLogout: () => void }) {
  return <main className="min-h-screen grid place-items-center bg-surface p-6"><section className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl text-center">
    <h1 className="text-2xl font-bold text-primary">Radar TS</h1>
    {state === 'loading' && <p className="mt-4">Verificando acesso…</p>}
    {state === 'signed-out' && <><p className="my-4 text-on-surface-variant">Entre com uma conta Google autorizada.</p><button className="rounded-lg bg-primary px-5 py-3 font-semibold text-white" onClick={onLogin}>Entrar com Google</button></>}
    {(state === 'denied' || state === 'error') && <><p className="my-4 text-error">{message || `A conta ${email || ''} não está autorizada.`}</p><button className="rounded-lg border px-4 py-2" onClick={onLogout}>Usar outra conta</button></>}
    {message && state === 'signed-out' && <p className="mt-3 text-sm text-error">{message}</p>}
  </section></main>;
}

function UsersPanel({ current, onClose }: { current: User; onClose: () => void }) {
  const [users, setUsers] = useState<AuthorizedUser[]>([]), [email, setEmail] = useState('');
  const [role, setRole] = useState<AccessRole>('usuario'), [message, setMessage] = useState('Carregando…');
  const load = async () => { try { const result = await getDocs(collection(db, USERS)); setUsers(result.docs.map(item => item.data() as AuthorizedUser).sort((a, b) => a.email.localeCompare(b.email))); setMessage(''); } catch { setMessage('Não foi possível carregar os usuários.'); } };
  useEffect(() => { void load(); }, []);
  async function save(event: FormEvent) {
    ...
    const now = new Date().toISOString();
    try { await setDoc(doc(db, USERS, normalized), { email: normalized, perfil: role, ativo: true, atualizadoEm: now, criadoEm: now, criadoPor: current.uid }, { merge: true }); setEmail(''); await load(); } catch { setMessage('Não foi possível salvar o usuário.'); }
  }
  async function toggle(item: AuthorizedUser) {
    if (isInitialAdmin(item.email)) return;
    try { await setDoc(doc(db, USERS, item.email), { ativo: !item.ativo, atualizadoEm: new Date().toISOString() }, { merge: true }); await load(); } catch { setMessage('Não foi possível alterar o acesso.'); }
  }
  return <div className="fixed inset-0 z-[110] grid place-items-center bg-black/40 p-4"><section className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl">
    ...
    <form className="my-5 flex flex-wrap gap-2" onSubmit={save}><input aria-label="E-mail" className="min-w-56 flex-1 rounded border p-2" placeholder="email@empresa.com" value={email} onChange={e => setEmail(e.target.value)} /><select aria-label="Perfil" className="rounded border p-2" value={role} onChange={e => setRole(e.target.value as AccessRole)}><option value="usuario">Usuário</option><option value="admin">Administrador</option></select><button className="rounded bg-primary px-4 text-white">Adicionar</button></form>
    {message && <p className="text-sm">{message}</p>}<div className="max-h-72 overflow-auto">{users.map(item => <div key={item.email} className="flex items-center justify-between border-t py-3"><div><strong>{item.email}</strong><p className="text-xs">{item.perfil} · {item.ativo ? 'ativo' : 'inativo'}</p></div><button disabled={isInitialAdmin(item.email)} className="rounded border px-3 py-1 disabled:opacity-50" onClick={() => void toggle(item)}>{item.ativo ? 'Desativar' : 'Ativar'}</button></div>)}</div>
  </section></div>;
}
