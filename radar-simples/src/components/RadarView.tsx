...
import { compareMunicipiosPorUltimaAtividade } from '../utils/atividadeMunicipio';
...
type OrdenacaoRadar = 'atividade' | 'tarefa' | 'visita' | 'nome';
...
export default function RadarView({ municipios, onAbrirMunicipio, onNovaDespesa, onVerRelatorio }: RadarViewProps) {
  ...
  const [ordenacao, setOrdenacao] = useState<OrdenacaoRadar>('atividade');
  ...
  const linhas: LinhaMunicipio[] = useMemo(() => {
    ...
    return municipios
      ...
      .sort((a, b) => {
        if (ordenacao === 'atividade') return compareMunicipiosPorUltimaAtividade(a, b);
        if (ordenacao !== 'nome') {
  ...
  return (
    <div className="flex flex-col gap-space-sm pt-space-xs pb-24">
      ...
      <label className="flex flex-col gap-1 text-label-md text-on-surface-variant">
        ...
        {ordenacao === 'atividade' && <span className="text-label-sm">Última atividade manual primeiro; GPS, rota e despesas não influenciam.</span>}
