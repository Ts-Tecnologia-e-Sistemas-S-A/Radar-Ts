...
As fontes falham independentemente: uma consulta pendente não elimina os resultados confirmados das demais. O servidor usa Node 24 com certificados confiáveis do sistema operacional, preservando a validação TLS. A função de diagnóstico tem limite de 180 segundos; os ZIPs do IDEB são lidos sequencialmente para limitar memória, com limites de tamanho comprimido e descomprimido.
